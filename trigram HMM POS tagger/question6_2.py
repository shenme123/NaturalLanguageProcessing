...
def low_freq_to_sets_2(total_freqs, datafile):
	...
	while line:
		...
		if line:
			...
			wd = fields[0]
			if total_freqs[wd]<5:
				if wd.isdigit() or re.match("^[0-9,],[0-9,]$", wd) is not None:
					datafile_revised.append("_Num_ %s" % fields[1])
...
if __name__ == "__main__":
	...
	train_data = file(sys.argv[1], "r")
	train_data_revised = low_freq_to_sets_2(train_total_freqs, train_data)
	# recount freqs	
	counter3 = Hmm(3)
	counter3.train_data(train_data_revised)
	train_freqs_re = []
	# write frequencies to file 'ner.counts_sets'
	...
	test_data_ = to_set_2(wordlist_train, test_data)
